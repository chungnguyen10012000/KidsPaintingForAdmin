...
const OrderForm: React.FC = () => {
    ...
    function saveForm(formState: IOrderFormState): void {
        if (selectedProduct) {
            ...
            dispatch(addNotification("Thêm lớp", ` ${formState.name.value}`));
        ...
        <Fragment>
            <div className="card shadow mb-4">
                <div className="card-header py-3">
                    <h6 className="m-0 font-weight-bold text-green">Tạo lớp</h6>
                </div>
                <div className="card-body">
                    <form onSubmit={saveOrder}>
                        ...
                        <button className="btn btn-danger" onClick={() => resetForm()}>Xóa</button>
                        <button type="submit" className={`btn btn-success left-margin ${getDisabledClass()}`}>Tạo</button>
                    </form>
                </div>
            </div>
        </Fragment>
